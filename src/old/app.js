        ...
        onKeypress(e) {
            ...
            if (val != undefined && val != '') {
                if(this.searchType == 'address') {
                    ...
                    fetch(api_admin.concat(field,val,"&limit=5&autocomplete=1"))
                    .then(res => res.json())
                    .then(res => {
                        let suggestions = [];
                        if(res) {
...
let pdfTemplate = {
    ...
    mounted() {
        ...
        setTimeout(() => {
            ...
            let map = L.map('map2').setView(coords,20);
    ...
    template:`
        <div class="container">
            <div style="padding:50px">
                <img src="img/logo_anct.png" style="width:40%;margin-bottom:20px">
                <h2 style="font-family:'Marianne-Bold'">{{ fs.lib_fs }}</h2>
            </div>
            <div class="row">
                <div class="col-sm-6">
                    <div class = "intro">
...
let card_template = {
    ...
    methods: {
        ...
        //         console.log(coucou);
        //         document.body.innerHTML = coucou.innerHTML;
                ...
                <div class="card-body"">
                  ...
                  <div class="corps" v-show="showInfo">
                    ...
                    <div class="card-controls">
                        <button type="button" class="card-btn btn btn-outline-primary btn" @click="zoomOnMap">
                            <i class="las la-search-plus"></i>
                            Zoom
                        </button>
                        <button type="button" class="card-btn btn btn-outline-primary btn" @click="print(fs)">
...
let sliderTemplate = {
    ...
    methods: {
        emitRadius() {
            params.set("radius",this.radiusVal);
    ...
    mounted() {
        if(params.has("radius")) {
            this.radiusVal = params.get("radius");
        } else {
            this.radiusVal = 10;
        };
        this.emitRadius();
    ...
    template:`
        <div>
            ...
            <input type="range" class="form-range" 
                   ...
                   v-model="radiusVal" 
                   @change="emitRadius" 
                   min="minRadiusVal" max="maxRadiusVal" step="0.2"style="width:100%">
        </div>
    `
}
...
let map_template = {
    ...
    methods: {
        ...
        checkURLParams() {
            ...
            switch (queryType) {
                case "address":
                    let resultMarker = params.get("qCoords").split(",");
    ...
    watch: {
        marker() {
            ...
            this.params.set('qCoords',this.marker);
            this.params.set('qLabel',this.marker_tooltip);
            this.params.set('radius',this.searchRadius);
            window.history.pushState({},'',this.url);
        },
        depFilter() {
            this.clearMap();
